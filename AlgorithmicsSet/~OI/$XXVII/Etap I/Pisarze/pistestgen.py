...
def main(group, testname, folder, seed):
    random.seed(seed)
...
def gen(group, cnt=None):
    x, y = [], []
    while True:
        xx, yy = main(group, None, '.', random.randint(0, 2**64 - 1))
        x += xx; y += yy
        if cnt is None or len(x) > cnt:
            x = x[:cnt]; y = y[:cnt]
            return x, y
...
if __name__ == "__main__":
    ...
    main(group, testname, folder, seed)
