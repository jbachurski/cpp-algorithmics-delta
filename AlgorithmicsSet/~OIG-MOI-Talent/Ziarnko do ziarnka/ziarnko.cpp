...
const size_t DIGITS_LONG = 256, DIGITS_BUFFER = 3;
...
struct number_t
{
    ...
    template<typename T>
    void operator= (const T& o) { set_to(o); }
    template<typename T>
    void operator+= (const T& o) { add(o); }
    template<typename T>
    void operator-= (const T& o) { subtract(o); }
    template<typename T>
    void operator*= (const T& o) { multiply(o); }
    template<typename T>
    void operator/= (const T& o) { divide(o); }
};
template<size_t DIGITS>
...
int main()
{
    number_t<4> number[128];
    for(uint64_t i = 0; i < 128; i++)
        number[i] = i;
    number_t<256> N(0);
    N.read();
    uint32_t m = N.size();
    for(uint32_t i = N.size(); i --> 0 and m == N.size();)
        if(N[i] != 0)
            m = i;
    if(m % 2 == 0)
        m++;
    number_t<256> C(0), P(0);
    for(uint32_t i = m + 2; (i -= 2) + 1 > 0;)
    {
        C *= number[100];
        C += number[10*N[i]];
        C += number[N[i-1]];
