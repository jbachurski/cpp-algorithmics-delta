...
const size_t MAX = 1100, FIX = 50, MAX_N = 1000, MAX_P = 11;
const int32_t CLAMP_LO = FIX-1, CLAMP_HI = FIX+MAX_N;
const int32_t SMAX = MAX, SFIX = FIX;

static uint64_t A[MAX][MAX], S[MAX+1][MAX+1], P[MAX][MAX][MAX_P][4];
...
uint64_t pyramid(int32_t x, int32_t y, int32_t k)
{
    ...
    {
        int32_t d = 1 << min(log2floor(k), MAX_P);
        int32_t px = x+(k-d)*dx_sign;
        if(0 <= px and px <= SMAX)
            r += P[y][px][log2floor(k)][s];
        if(k - d > 0)
...
int main()
{
    ...
    for(int32_t p = 1, k = 2; k <= (int32_t)n and p < (int32_t)MAX_P; p++, k *= 2)
    {
