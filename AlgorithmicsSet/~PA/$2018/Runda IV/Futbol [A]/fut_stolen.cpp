...
namespace ntt
{
    ...
    void convolve(mod_t* A, int s1, mod_t* B, int s2, bool cyclic=false)
    {
        ...
        {
            fill(B + s2, B + csize, 0);
            ntt_dit4(B, csize, 1, roots);
            rep(i, csize) A[i] *= B[i];
        }
        ntt_dit4(A, csize, -1, roots);
        mod_t inv = mod_t(csize).inverse();
        rep(i, cyclic ? csize : s) A[i] *= inv;
    }
